    ...
    -- Lấy số lượng Gold nhận được sau khi RoundOver hiển thị
    local function getGoldReceived()
        local player = game:GetService("Players").LocalPlayer
    ...
    local function sendRequest(requestFunction)
        ...
        local goldReceived = getGoldReceived()
    
        local jsonBody = HttpService:JSONEncode({
            ["content"] = "",
            ["embeds"] = {{
                ["title"] = "Ultimate Tower Defense",  -- Tiêu đề chính
                ["type"] = "rich",
                ["color"] = tonumber(0xffffff),
                ["fields"] = {{
                    ["name"] = "Player Stats",  -- Tiêu đề Player Stats
                    ["value"] = "Gold: " .. currentGold .. "\nGem: " .. currentGem,
                    ["inline"] = true  -- Hiển thị bên trái
                }, {
                    ["name"] = "Rewards",  -- Tiêu đề Rewards
                    ["value"] = "" .. goldReceived,
